...
const DEFAULTS = {
    ...
    bgImage: 'https://tse4.mm.bing.net/th/id/OIP.qV4A3aMsaAxIj3XuzDFBZgHaEU?w=804&h=469&rs=1&pid=ImgDetMain&o=7&rm=3',
};
...
const HeroSection = () => {
    ...
    useEffect(() => {
        ...
        const load = () => {
            ...
            // Fetch central PDF links to override static paths
            fetchContent('pdfs').then(data => {
                if (!cancelled && data && data.brochure) {
                    setHero(prev => ({ 
                        ...prev, 
                        // If current brochureUrl is default or missing, or if we want to honor global uploads
                        brochureUrl: data.brochure || prev.brochureUrl 
                    }));
                }
            });
