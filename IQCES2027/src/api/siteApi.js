// API service for iqce2027 website
// Fetches live data from the shared dashboard backend

const BASE_URL = '/api';

// ── This must ALWAYS be 'iqce2027' for this conference site ──
const CONFERENCE_ID = 'iqce2027';
...
async function get(endpoint) {
    try {
        ...
        console.log(`[SiteAPI-IQCES] GET request to: ${url}`);
        ...
        console.error(`[SiteAPI-IQCES] GET ${endpoint} failed:`, e.message);
...
export async function submitRegistration(data) {
    ...
    try {
        console.log(`[SiteAPI-IQCES] POST request to: ${url}`);
        const res = await fetch(url, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        console.error('[SiteAPI-IQCES] submitRegistration failed:', e.message);
        throw new Error(`Connection Error: ${e.message} (Is the backend at ${BASE_URL} reachable?)`);
    }
}
...
export async function validateDiscountCode(coupon) {
    try {
        ...
        console.error('[SiteAPI-IQCES] Discount validate failed:', e.message);
